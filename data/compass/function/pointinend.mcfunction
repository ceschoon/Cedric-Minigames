...
execute at @s run summon minecraft:item ~ ~ ~ {Item:{id:"minecraft:compass",count:1,components:{lore:['{"text":"Dimension: The End","italic":false}']}}}

execute at @s run data merge entity @e[type=item,nbt={Item:{id:"minecraft:compass"}},distance=..3,limit=1] {Item:{components:{"minecraft:lodestone_tracker":{target:{dimension:"minecraft:the_end",pos:[I;0,0,0]},tracked:false}}}}
