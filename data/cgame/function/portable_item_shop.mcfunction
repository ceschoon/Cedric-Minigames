...
execute as @a[nbt={SelectedItem:{components:{"minecraft:enchantment_glint_override":1b,"minecraft:lore":['{"bold":false,"italic":false,"text":"Portable Item Shop","underlined":false}']}}}] at @s unless entity @e[type=villager,tag=cgame_portable_item_shop,distance=..3] run summon villager ~ ~ ~ {Tags:['cgame_portable_item_shop'],VillagerData:{profession:cleric,level:5,type:savanna},NoAI:1,Invulnerable:1,Silent:1,active_effects:[{id:invisibility,duration:-1,show_particles:0b}],Offers:{Recipes:[{buy:{id:amethyst_shard,count:2,components:{"minecraft:enchantment_glint_override":1b,"minecraft:lore":['{"italic":false,"text":"Life crystal"}']}},sell:{id:bell,count:1},xp:1,maxUses:999999}]}}
