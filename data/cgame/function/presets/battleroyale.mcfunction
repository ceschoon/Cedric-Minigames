
scoreboard players set #cgame_preptime cgame_setting 0
scoreboard players set #cgame_time_to_win cgame_setting 999999
scoreboard players set #cgame_score_to_win cgame_setting 1200
scoreboard players set #cgame_curse_active cgame_setting 0
scoreboard players set #cgame_include_barrel_drops cgame_setting 1
scoreboard players set #cgame_map_size cgame_setting 3
scoreboard players set #cgame_shrink_border cgame_setting 1
scoreboard players set #cgame_include_hunter_item_shop cgame_setting 1

scoreboard players set @a[scores={cfp_is_fake_player=0}] cgame_starting_team 4

