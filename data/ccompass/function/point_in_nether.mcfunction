...
execute at @s run summon minecraft:item ~ ~ ~ {Item:{id:"minecraft:compass",count:1,components:{lore:['{"text":"Dimension: The Nether","italic":false}','{"text":"Put me in a crafting table to change the target","italic":false}']}}}

execute at @s run data merge entity @e[type=item,nbt={Item:{id:"minecraft:compass"}},distance=..1,limit=1] {Item:{components:{"minecraft:lodestone_tracker":{target:{dimension:"minecraft:the_nether",pos:[I;0,0,0]},tracked:false}}}}
